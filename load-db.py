...
while line:
	...
	#print(annotations)
	v = run.Variant(id = v_index+1, chr = fields[0], pos = fields[1], rsid = fields[2], ref= fields[3], alt = fields[4], AC_AFR = info_field['AC_AFR'], AN_AFR = info_field['AN_AFR'], AC_AMR = info_field['AC_AMR'], AN_AMR = info_field['AN_AMR'], AC_EAS = info_field['AC_EAS'], AN_EAS = info_field['AN_EAS'], AC_FIN = info_field['AC_FIN'], AN_FIN = info_field['AN_FIN'], AC_NFE = info_field['AC_NFE'], AN_NFE = info_field['AN_NFE'], AC_OTH =  info_field['AC_OTH'], AN_OTH = info_field['AN_OTH'], AC_SAS = info_field['AC_SAS'], AN_SAS = info_field['AN_SAS'])
	run.db.session.add(v)
	
	for annot in annotations:
		impact = annot['IMPACT']
		if impact in set(["HIGH", "MODERATE", "LOW"]):
			a = run.Annotation(id = a_index+1, variant_id = v_index+1, allele = annot['Allele'], consequence = annot['Consequence'], symbol = annot['SYMBOL'], gene = annot['Gene'], lof = annot["LoF"], lof_filter = annot['LoF_filter'], lof_flags = annot['LoF_flags'])
			run.db.session.add(a)
			a_index = a_index+1
	if index % 1000 ==0:
		# commit only every 1000 variants
